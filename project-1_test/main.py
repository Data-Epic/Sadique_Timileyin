...
def spreadsheet_format(filename, sheet_name, first_column, second_column):
    ...
    if (
        ...
        and isinstance(first_column, str)
        and isinstance(second_column, str)
    ):
        ...
        raise ValueError(
            "The input expected a string object e.g 'A1' for the cols and rows input."
...
def new_worksheet(filename, new_name, rows, columns):
    ...
    if (
        ...
        raise ValueError(
            "The input expected a number/integer for the "
...
if __name__ == "__main__":
    ...
    links = driver.find_elements(
        By.CSS_SELECTOR, value=".directory-content-box-inner a"
    )
    for link in links:
    ...
    for links in linked:
        ...
        while not company_link:
            try:
                linke = driver.find_element(
                    By.XPATH,
                    value="/html/body/main/div[1]/div/section[1]/div/div[1]"
                    "/div/div[2]/a",
                )
        ...
        while not linkedin:
            try:
                texts = driver.find_element(
                    By.CSS_SELECTOR, value=".company-details-socials a"
                )
